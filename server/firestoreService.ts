...
export class FirestoreService {
  ...
  static async createTenant(newTenant: Partial<TenantConfig> & { initialAdminEmail?: string; adminEmail?: string; domain?: string }): Promise<TenantConfig> {
    ...
    const domainName = newTenant.domain || newTenant.defaultDomain || `${tenantId}.marketlane.app`;
    const currency = (newTenant.currency || 'GBP').toUpperCase();
    const currencySymbol =
      newTenant.currencySymbol ||
      ({ GBP: '£', EUR: '€', USD: '
      tenantId,
