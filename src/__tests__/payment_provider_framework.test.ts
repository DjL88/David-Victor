...
describe('Round 2 payment provider framework', () => {
  ...
  it('preserves the existing active-profile DPay default during provider-registry migration', async () => {
    ...
    expect(resolved.providerId).toBe('deliverect_dpay');
    expect(resolved.selectionSource).toBe('legacy_dpay_default');
  });
