...
const LeafletMapInner: React.FC<StoreLocationMapProps> = ({
  ...
  useEffect(() => {
    ...
    const defaultLat = userCoordinates?.latitude || stores[0]?.coordinates?.latitude || 51.5074;
    ...
    // THE FIX FOR THE HALF-CUT MAP:
    // Repeatedly force Leaflet to recalculate tiles while the modal transition finishes
  ...
  useEffect(() => {
    ...
    if (userCoordinates && typeof userCoordinates.latitude === 'number') {
      ...
      userMarker.bindTooltip(userAddress?.line1 || userAddress?.street || 'Your Location', {
        direction: 'top',
        offset: [0, -10],
    ...
    prioritizedStores.forEach((store, index) => {
      ...
      const ringClass = isSelected ? 'ring-4 ring-emerald-400/50 scale-115' : 'ring-0';
      ...
      storeMarker.on('click', () => setActiveStoreForPopup(store));
  ...
  return (
    <div
      ...
      className={`relative w-full rounded-2xl border border-gray-200/80 shadow-xs ${className}`}
      style={{ height, overflow: 'hidden' }}
    >
      <style>{`
        .leaflet-interactive { cursor: pointer !important; }
        .custom-user-pin, .custom-store-pin { background: transparent !important; border: none !important; outline: none !important; }
        .leaflet-tooltip { background: white !important; border: 1px solid rgba(229, 231, 235, 0.8) !important; border-radius: 8px !important; color: #111827 !important; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05) !important; }
      ...
      <div className="absolute top-2 left-2 z-[400] bg-white/95 backdrop-blur-xs px-3 py-1.5 rounded-xl border border-gray-200/80 shadow-2xs text-[10px] font-semibold text-gray-700 flex flex-wrap items-center gap-3 pointer-events-none">
        ...
        {radiusExpanded && <span className="text-[9px] text-gray-400 italic">(Zone radius extended)</span>}
      </div>
      ...
      {activeStoreForPopup && (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <h4 className="text-xs font-bold text-gray-900 truncate">{activeStoreForPopup.name}</h4>
              </div>
              <p className="text-[11px] text-gray-500 truncate mt-0.5">
                {activeStoreForPopup.address?.line1 || activeStoreForPopup.address?.street}, {activeStoreForPopup.address?.city}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="flex items-center justify-between text-[11px] py-1.5 px-2.5 rounded-xl bg-gray-50 border border-gray-100">
            <span className="font-semibold text-gray-900">{formatDistanceHuman(activeStoreForPopup.distanceMeters)} away</span>
            <span className="flex items-center gap-1 text-gray-600">
              {activeStoreForPopup.isDeliveryServiceable ? (
                <><Truck className="w-3.5 h-3.5 text-emerald-600" /><span>{activeStoreForPopup.deliveryEta || '15–25 min'}</span></>
              ) : activeStoreForPopup.isCollectionAvailable ? (
                <><ShoppingBag className="w-3.5 h-3.5 text-amber-600" /><span>Collect in 10 min</span></>
              ) : (
                <><Clock className="w-3.5 h-3.5 text-gray-400" /><span>Closed</span></>
              )}
            </span>
          </div>
          ...
          {onSelectStore && (
            <button
              ...
              className={`w-full py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                !activeStoreForPopup.isOpen ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : selectedStore?.id === activeStoreForPopup.id ? 'bg-gray-200 text-gray-800' : 'text-white shadow-2xs hover:brightness-105'
              }`}
            >
              {selectedStore?.id === activeStoreForPopup.id ? <><Check className="w-3.5 h-3.5" /><span>Active Store</span></> : activeStoreForPopup.isOpen ? <span>Select this store</span> : <span>Currently closed</span>}
            </button>
          )}
        </div>
      )}
...
export const StoreListFallbackView: React.FC<StoreLocationMapProps> = ({ userCoordinates, userAddress, stores, selectedStore, onSelectStore, height = '320px', className = '', zoneRadiusMeters = DEFAULT_ZONE_RADIUS_METERS, dispatchServiceabilityMap }) => {
  const { primaryBtnStyle } = useTenantStyles();
  const { prioritizedStores } = useMemo(() => prioritizeStoresForCustomer(stores, userCoordinates, { zoneRadiusMeters, maxStores: MAX_MAP_STORES, dispatchServiceabilityMap }), [stores, userCoordinates, zoneRadiusMeters, dispatchServiceabilityMap]);

  return (
    <div id="store-list-fallback-view" className={`relative w-full rounded-2xl bg-white border border-gray-200 shadow-xs flex flex-col justify-between overflow-hidden ${className}`} style={{ minHeight: height, maxHeight: typeof height === 'number' ? `${height}px` : height }}>
      <div className="flex-1 overflow-y-auto p-3 space-y-2 divide-y divide-gray-50">
        {prioritizedStores.length === 0 ? (
          <div className="py-8 text-center text-xs text-gray-400">No nearby stores found for this location.</div>
        ) : (
          prioritizedStores.map((store) => {
            const isSelected = selectedStore?.id === store.id;
            return (
              <div key={store.id} onClick={() => { if (onSelectStore && store.isOpen) onSelectStore(store); }} className={`pt-2 first:pt-0 flex items-center justify-between gap-3 p-2.5 rounded-xl transition-all cursor-pointer ${isSelected ? 'bg-gray-50 border border-gray-300 shadow-2xs' : 'hover:bg-gray-50 border border-transparent'}`}>
                <div className="flex items-center gap-2.5 min-w-0">
                  <div style={isSelected ? primaryBtnStyle : undefined} className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? 'text-white' : !store.isOpen ? 'bg-gray-100 text-gray-400' : store.isDeliveryServiceable ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                    <StoreIcon className="w-4 h-4" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-gray-900 truncate">{store.name}</p>
                    <p className="text-[11px] text-gray-500 truncate">{store.address.line1 || store.address.street} • {formatDistanceHuman(store.distanceMeters)}</p>
                  </div>
                </div>
              </div>
            );
