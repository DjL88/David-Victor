...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const depositDisplay = `${currencySymbol}${(depositAmount || 0).toFixed(2)}`;
            ...
            <div className="flex flex-wrap items-baseline gap-2 mb-4">
              ...
              {hasDeposit && (
                <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">
                  {t('product.includesDeposit').replace('{amount}', depositDisplay)}
                </span>
              )}
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {(() => {
                      ...
                      return t('product.availableStores').replace('{available}', String(avail)).replace('{total}', String(total));
                    })()}
                  </span>
                </div>
            ...
            {hasNutrition && (
              <div className="mb-5">
                <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                  {t('product.nutritionPer').replace('{portion}', product.nutritionalInfo!.portionSize || '100g/ml')}
                </h2>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  {hasCalories && (
                    <div className="p-2.5 rounded-xl bg-gray-50 border border-gray-100">
                      <span className="text-gray-400 block text-[10px]">{t('product.energy')}</span>
            ...
            {hasAlcohol && (
              <div className="mb-5 p-3.5 rounded-2xl bg-purple-50/80 border border-purple-100 text-xs text-purple-950">
                ...
                <div className="space-y-1 text-purple-900">
                  {product.beverageInfo?.alcoholByVolume !== undefined && (
                    <p>
                      <strong>{t('product.abv')}:</strong> {product.beverageInfo.alcoholByVolume}% vol
                    </p>
                  )}
                  <p>
                    <strong>{t('product.legalNotice')}:</strong> {t('product.ageNotice')}
                  </p>
                </div>
              </div>
            )}
            ...
            {hasDeposit && (
              <div className="mb-5 p-3.5 rounded-2xl bg-emerald-50/80 border border-emerald-100 text-xs text-emerald-950">
                <div className="flex items-center gap-2 font-bold mb-1">
                  ...
                  <span>{t('product.drs')}</span>
                </div>
                <p className="text-emerald-900 leading-relaxed">
                  {t('product.drsNotice').replace('{amount}', depositDisplay)}
                </p>
              </div>
            )}
