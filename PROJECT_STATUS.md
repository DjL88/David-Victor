...
1. **LIVE DELIVERECT INTEGRATION (Operational)**:
   ...
   - **Fulfillment Capability Normalization & Admin Test Order Tool Hardening**: Refactored `LinkedAccountsAdapter.ts` store capability normalization to implement the authoritative 4-tier priority hierarchy (`fulfillmentTypes` -> `settings.<type>.enabled` -> legacy `fulfillmentCapabilities` / top-level flags -> unknown/false). Removed fail-open `?? true` defaults in `DeliverectApiClient.ts` so unknown store capabilities never default to `true`. Hardened the Admin Test Order tool in `v1Router.ts` and `IntegrationsAdminScreen.tsx` to inspect selected store's fulfillment capabilities before posting basket, returning 400 Bad Request with "Collection is not enabled for this Deliverect Commerce store." if pickup is unsupported. Added comprehensive unit tests in `src/__tests__/fulfillment_capabilities.test.ts`.
