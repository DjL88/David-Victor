...
- **SEC-02b (2026-09-24):** Privileged Admin routes have centralized staging/production MFA and Firebase App Check enforcement behind explicit rollout flags. The web client now supports Firebase TOTP enrollment/challenge and sends App Check tokens on Admin/Commerce requests; checkout App Check has an independent rollout flag so customer traffic can be cut over safely. Existing deployments remain unaffected until Identity Platform TOTP, reCAPTCHA Enterprise App Check, `VITE_FIREBASE_APPCHECK_SITE_KEY`, and the corresponding runtime flags are provisioned. Plaintext bootstrap-admin configuration is not committed.
- **SEC-01 storage follow-up (merged as #102, 2026-09-24):** Shared test-bucket rules are fail-closed for writes across legacy non-David-Victor application paths. Existing reads are preserved for compatibility; David-Victor production remains blocked on SEC-11 dedicated storage isolation.
- **SEC-03 webhook security (2026-09-24):** Deliverect callbacks are route-bound to a Firestore-provisioned identifier; inbound HMAC uses only the tenant canonical `deliverect-webhook-<tenantId>` Secret Manager secret; cross-tenant secret scanning, hostname/account routing and implicit staging channel/location secrets are removed. Staging fallback requires an explicit flag and is forbidden in production or while any tenant is live. Replay identity is `sha256(tenantId + rawBody)`; timestamp freshness, route-specific body limits and an authenticated per-tenant ingress bucket are enforced. A separate production App Hosting config excludes preview/staging flags.
- **SEC-04a Cloud Tasks authentication (2026-09-24):** Cloud Tasks worker execution remains fail-closed on explicit `CLOUD_TASKS_SA_EMAIL` and exact `CLOUD_TASKS_AUDIENCE`; the audience is also the canonical worker origin. Worker OIDC verification requires Google issuer, exact audience, exact service-account email and `email_verified=true`. Optional Cloud Tasks configuration no longer blocks the storefront/Admin HTTP service from binding its port; enqueue/worker paths enforce the security contract when used.
- **INT-01 Commerce mapping durability (2026-09-24):** A successful Deliverect Commerce response containing zero stores is treated as transient/degraded evidence, not authoritative disconnection. The adapter preserves any persisted last-known-good store/location/catalogue mapping, skips destructive Firestore reconciliation, returns `STALE_LAST_KNOWN_GOOD`, and leaves explicit admin disconnect/reassignment or later positive non-empty upstream evidence as the removal authorities. Regression coverage exercises both persisted-LKG and first-empty-discovery cases.
- **SEC-04b payment-state safety (2026-09-24):** DPay circuit state is isolated by tenant/service and non-retryable 4xx request errors do not poison upstream health. Cancelling an uncaptured authorization is provider-first: local payment/order projections change only after the provider confirms release. The live Deliverect adapter deliberately returns 501 until the partner-specific void/release contract is verified, preventing false local cancellation. Remaining SEC-04b checkout idempotency, tokenization proxy, hosted redirect policy and unpaid-payment policy are still being reconciled separately from stale PR #111.

- **DV-07a (2026-09-24):** Retail/Quest order host, path template and allowlisted `x-deliverect-version` header are configurable per tenant by platform superadmins, with environment fallback, diagnostics and per-order endpoint trace metadata; staging A0 remains explicitly pinned to `https://api.staging.deliverect.io/{channelName}/order/{channelLinkId}`.
