...
- **SEC-04a Cloud Tasks authentication (2026-09-24):** Live startup requires explicit `CLOUD_TASKS_SA_EMAIL`, `CLOUD_TASKS_AUDIENCE` and `APP_URL`. Worker OIDC verification requires Google issuer, exact audience, exact service-account email and `email_verified=true`. Settlement, cancellation, realtime picking and bulk-menu queues mint tokens using the same constants; localhost/example/Cloud Run/BFF identity fallbacks are removed from live dispatch. Staging must provision these runtime values before this package is merged/deployed.
