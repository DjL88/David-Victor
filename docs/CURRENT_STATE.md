...
- **SEC-04a (audit-defined Cloud Tasks auth, 2026-09-24):** Live startup requires explicit `CLOUD_TASKS_SA_EMAIL`, `CLOUD_TASKS_AUDIENCE` and `APP_URL`; every worker verifies issuer, exact audience, exact service-account email and `email_verified`; settlement, cancellation, realtime picker and bulk-menu queues all mint OIDC tokens with that same identity/audience. Localhost/example.com/Cloud Run audience fallbacks are removed from live task dispatch. Earlier PR #103 was a webhook-HMAC prerequisite, not this audit package.
