...
- SEC-02b — Admin authorization can require Firebase TOTP MFA; the Admin login surface supports TOTP enrolment and second-factor challenges. Firebase App Check tokens are attached by the web clients and verified on all /admin routes plus POST /checkouts. Production defaults fail closed; staging enforcement is enabled with ADMIN_MFA_ENFORCEMENT=required and APP_CHECK_ENFORCEMENT=required after Identity Platform TOTP and the reCAPTCHA Enterprise App Check key are enabled. Plaintext bootstrap-admin configuration was removed from App Hosting. Firebase-console provider/App Check registration remains an external deployment prerequisite. (2026-09-24)
- Audit ID note — merged PRs #102–#104 added shared-bucket and webhook prerequisites, but they do not close the audit-defined SEC-03 (tenant-secret/replay/limits), SEC-04a (Cloud Tasks OIDC) or SEC-04b (payments) packages; those remain separate work packages.
