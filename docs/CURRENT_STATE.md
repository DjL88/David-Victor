...
- **SEC-04b payments (2026-09-24):** Tenant payment policy defaults unpaid checkout to false; the demo/pay-at-store case opts in explicitly. Basket checkout is atomically claimed in `checkoutLocks` before Deliverect order creation. DPay Create Token is proxied through the documented Basis Theory boundary without storing card data; fake 4242 tokenization is removed. Hosted redirects require a tenant HTTPS-origin allowlist and canonical checkout-return route. Commerce/DPay circuit state is tenant-scoped and ignores 4xx. Authorization cancellation mutates Firestore only after a provider-confirmed void; the live DPay void operation remains fail-closed until Deliverect confirms the partner-specific endpoint/contract.
