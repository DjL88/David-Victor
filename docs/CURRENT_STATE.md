...
- **SEC-03 prerequisite (2026-09-24):** Shared test-bucket rules are fail-closed for writes across legacy non-David-Victor application paths. Existing reads are preserved for compatibility; David-Victor production remains blocked on SEC-11 dedicated storage isolation.
- **SEC-03 (2026-09-24):** Deliverect webhook tenant routing is path-identifier-only; live HMAC verification uses the canonical per-tenant `deliverect-webhook-<tenantId>` secret with no all-tenant scan/global fallback; staging channel/location HMACs are explicit-flag only and blocked in production/live tenants; replay identity is SHA-256 over tenant + exact raw bytes; stale timestamps are rejected; route-specific body limits and per-tenant queue backpressure are enforced; production App Hosting config excludes staging/preview switches.
