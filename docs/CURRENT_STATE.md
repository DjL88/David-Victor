...
- **Audit SEC-04a Cloud Tasks auth (2026-09-24):** Live runtimes require explicit `CLOUD_TASKS_SA_EMAIL`, `CLOUD_TASKS_AUDIENCE` and `APP_URL`; worker OIDC verifies issuer, exact audience, exact verified service-account email; enqueue uses those same constants with no localhost/example fallbacks. This package must not merge into staging until those runtime values are provisioned.
