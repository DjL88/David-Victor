...
- **SEC-04b payment/checkout safety (2026-09-24):** DPay circuit state is isolated by tenant/service and non-retryable 4xx request errors do not poison upstream health. Cancelling an uncaptured authorization is provider-first: local payment/order projections change only after the provider confirms release. The live Deliverect adapter deliberately returns 501 until the partner-specific void/release contract is verified. Checkout now atomically claims a tenant+basket lock before the first upstream order-creation request, recovers completed attempts, permits retries after failed attempts, and fails closed on concurrent creation. Remaining SEC-04b tokenization proxy, hosted redirect policy and unpaid-payment policy are still being reconciled separately from stale PR #111.
